...
public class _01_Instance {
	...
	public static void main(String[] args) {
		...
		//2. 정의한 클래스 타입의 값을 만들어주는 작업을 '인스턴스화'라고 함
		//인스턴스화를 진행하게 되면 클래스내의 선언된 변수들과 메소드들이 메모리에 올라가고
		//그 변수들과 메소드를 사용할 수 있는 클래스타입의 값이 객체로 만들어지게 된다.
		//인스턴스화 방식: new 생성자();
		car = new Car();
		...
		//car와 car2는 서로 영향을 주지 않음!!!
		Car car2 = new Car();
		
		System.out.println(car2); //car과 주소값 다름
		System.out.println(car2.company); //null
		
		Car car3 = generateCar(car);
		car3.printCarInfo();
	}
	
	//클래스는 타입의 역할도 하기 때문에 메소드의 리턴타입이나 매개변수로도 사용 가능
